...
namespace bombersquad_ai
{
	namespace fsm
	{
		namespace states
		{
			public class SurroundState : State<BomberAgent>
			{
				...
				public SurroundState (GameState gs, bool isBombPassable)
				{
					this.gs = gs;
				...
				*/
				...
				public Action getAction (BomberAgent owner)
				{
					...
					Coords coord = surroundCoords [owner.AgentId];
					//foreach (Coords coord in surroundCoords) {
					//this.targets [i] = coord;
					this.target = coord;
					Coords aiPos = this.gs.GetAgentCoords (owner.AgentId + 1);
					List<Coords> path = pathPlan.calculateMovementPlan (g, aiPos, coord);
					if (path.Count > 0) {
						Coords[] pathCoords = path.ToArray ();
						move = MovementUtil.getMovementActionType (aiPos, pathCoords [0]);
					} else {
						Console.WriteLine ("Agent " + owner.AgentId + " cannot execute SurroundState");
						this.cannotExecuteStrategy = true;
					}
					//i++;
					//}
